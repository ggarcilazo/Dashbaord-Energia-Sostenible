...
const firebaseConfig = {
    ...
    databaseURL: "https://energia-responsable-ucv-default-rtdb.asia-southeast1.firebasedatabase.app",
    projectId: "energia-responsable-ucv",
    storageBucket: "energia-responsable-ucv.firebasestorage.app",
...
// -----------------------------
// UTIL: calcular promedios y cachear consumo
// -----------------------------
function computeAveragesAndCache(consumoData) {
    window.fullConsumoDataCache = consumoData || {};

    window.promedioHistoricoConsumo = {};
    window.ultimoConsumoMensual = {};

    for (const rawPab in window.fullConsumoDataCache) {
        const pabKey = rawPab.toUpperCase();
        const pabData = window.fullConsumoDataCache[rawPab];
        ...
        for (const year of Object.keys(pabData).sort()) {
            const monthsObj = pabData[year];
            for (const month of Object.keys(monthsObj).sort()) {
                const registro = monthsObj[month];
                if (!registro) continue;

                const consumo = Number(registro.consumption) || 0;
                if (consumo > 0) {
                    sum += consumo;
                    count++;
                }
                ...
                const stamp = Number(`${year}${m}`);
                if (stamp >= ultimoStamp) {
                    ultimoStamp = stamp;
                    ultimoValor = consumo;
                }
            }
        ...
        window.promedioHistoricoConsumo[pabKey] = count > 0 ? (sum / count) : 0;
        window.ultimoConsumoMensual[pabKey] = ultimoValor;
    }

    console.log("✅ promedioHistoricoConsumo:", window.promedioHistoricoConsumo);
    console.log("✅ ultimoConsumoMensual:", window.ultimoConsumoMensual);
    // 🔔 Revisar alertas automáticamente cuando se actualiza el promedio
    if (typeof window.checkForEnergyAlerts === "function") {
        window.checkForEnergyAlerts();
    }
    // 🏆 Actualizar ranking gamificado
    if (typeof window.updateGamificationRanking === "function") {
        window.updateGamificationRanking();
    }
}
 ...
 * y luego creando el PDF para forzar la descarga directa (compatible con PC, Android e iPhone/iPad).
 */
async function generatePdfReport() {
    try {
        ...
        // === GENERACIÓN DEL PDF (COMPATIBLE UNIVERSALMENTE) ===
        ...
        html2canvas(tempContainer, {
            ...
            .then(canvas => {
                ...
                // ✅ Detectar dispositivo móvil y abrir correctamente
                if (/Android|iPhone|iPad|iPod/i.test(navigator.userAgent)) {
                    link.target = '_blank'; // Evita bloqueos de descarga
                }
...
function updateRankingChartByMonth(selectedDate, fullData) {
    ...
    if (typeof window.updateMonthlyKwhTotal === 'function') {
        ...
        // 🔧 FIX: usar rankingLabels en lugar de participatingBuildings
        const labels = rankingLabels.map(label => {
            // Convertir "Pabellón A" → "A"
            return label.replace("Pabellón", "").trim().toUpperCase();
        });

        const consumoActualArray = [];
        const consumoPromedioArray = [];

        labels.forEach(pab => {
            const consumoMes = window.ultimoConsumoMensual[pab] || 0;
            const promedioHist = window.promedioHistoricoConsumo[pab] || 0;

            consumoActualArray.push(consumoMes);
            consumoPromedioArray.push(promedioHist);
        });

        // Si existe el gráfico comparativo mensual, actualizarlo
        if (typeof window.initMonthlyComparisonChart === "function") {
            window.initMonthlyComparisonChart(labels, consumoActualArray, consumoPromedioArray);
        }
    ...
    // 🔔 Activar alertas
    if (typeof window.checkForEnergyAlerts === "function") window.checkForEnergyAlerts();
...
function fetchDataForRankingChart() {
    ...
    onValue(consumoRef, (snapshot) => {
        ...
        // ⬅️ Aquí actualizamos promedios y último consumo
        computeAveragesAndCache(full);

        // ⬅️ Esto mantiene tu dashboard funcionando igual
        populateRankingFilter(full);

    }, (error) => {
        console.error("Error al leer /consumo_mensual para ranking:", error);

        computeAveragesAndCache({});
    ...
    if (uploadForm) {
        uploadForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                // 🔥 Recalcular promedios, ranking y alertas después de guardar datos
                setTimeout(() => {
                    setupDashboardDataListener();
                }, 600);
